...
async function fullResponse(request: Request, bucket: R2Bucket): Promise<Response> {
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (condition) {
      return new Response(null, { status: condition, headers: installerHeaders(metadata) })
    }
