...
export const DEMO_FILES: Record<Kind, DemoFile> = Object.fromEntries(
  demoManifest.items.map((it) => [
    it.kind,
    { file: it.file, bytes: it.bytes, sizeLabel: it.sizeLabel },
  ]),
...
const SAMPLE_IMAGE_SHARE = `${SITE_URL}/share/site/background-b5e56005010603d6f444855406a91f70`;
const REAL_SHARES: Record<Kind, string> = {
  movie: `${SITE_URL}/share/site/earth-asset-59db73ad1efb4053ee4415778402cc86`,
  image: SAMPLE_IMAGE_SHARE,
  audio: `${SHARE_BASE}/sample-wav-e9bman`,
  markdown: `${SHARE_BASE}/sample-markdown-d7ja0j`,
};
const REAL_FILE_NAMES: Partial<Record<Kind, string>> = {
  image: "background.png",
  movie: "earth-asset.mp4",
};

export function linksFor(row: RowID): {
  name: string;
  page: string;
  file: string | null;
} {
  if (row === "seed") {
    ...
    return {
      ...
      file: `${SAMPLE_IMAGE_SHARE}/${REAL_FILE_NAMES.image ?? sampleImage}`,
    };
  }
  ...
  return {
    name: f,
    page: `${real}/index.html`,
    file: `${real}/${REAL_FILE_NAMES[row] ?? f}`,
  };
}
