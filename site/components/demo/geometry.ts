...
/* Chips float in the hero's negative space along a gentle arc from the
   ...
   passes behind them and gestures at the drop zone. Coordinates live in the
   arrow SVG's 1100×637 viewBox (non-scaling strokes keep lines crisp), so chips
   and arrow endpoints stay glued together at any width. */
...
/* Rendered size of an arc chip, in the arrow's 1100×637 viewBox units.
   Keep in sync with `.chip-arc .chip` width/height in chips.css — the arc
   layer is authored in those same units, so the two must match. */
export const ARC_CHIP = { width: 74, height: 89 };
...
export function arcChipStyle(kind: Kind): CSSProperties {
  ...
  const rot = arrowTangentDeg(t);
  return {
    // center the chip on its point on the curve
    left: `${(p.x - ARC_CHIP.width / 2).toFixed(1)}px`,
    top: `${(p.y - ARC_CHIP.height / 2).toFixed(1)}px`,
