...
function captureInitialAttribution(client: Mixpanel): void {
  ...
  for (const [k, v] of Object.entries(currentUtms())) attribution[`initial_${k}`] = v
...
export async function initAnalytics(token: string | undefined): Promise<boolean> {
...
export const analytics = {
  track(event: string, properties?: Properties): void {
    if (enabled) mixpanel?.track(event, stripUndefined({ ...pageProperties(), ...properties }))
  },
  trackPageView(): void {
